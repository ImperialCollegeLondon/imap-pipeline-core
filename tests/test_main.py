...
@pytest.mark.parametrize(
    ...
    [
        (
            "MAG_HSK_PW.pkts",
            "data/imap/mag/hsk-pw/2025/05/imap_mag_hsk-pw_20250502_v000.csv",
        ),
        (
...
def test_process_error_with_unsupported_file_type():
    ...
    assert not Path(
        "data/imap/mag/l1a/2025/05/imap_mag_l1a_norm-mago_20250502_v000.cdf"
    ).exists()
...
def test_fetch_binary_downloads_hk_from_webpoda(wiremock_manager, mode):
    ...
    assert result.exit_code == 0
    assert Path(
        "data/imap/mag/hsk-pw/2025/05/imap_mag_hsk-pw_20250502_v000.pkts"
    ).exists()

    with (
        open(
            "data/imap/mag/hsk-pw/2025/05/imap_mag_hsk-pw_20250502_v000.pkts", "rb"
        ) as output,
...
def test_fetch_science_downloads_cdf_from_sdc(wiremock_manager):
    ...
    assert result.exit_code == 0
    assert Path(
        "data/imap/mag/l1b/2025/05/imap_mag_l1b_norm-magi_20250502_v000.cdf"
    ).exists()

    with (
        open(
            "data/imap/mag/l1b/2025/05/imap_mag_l1b_norm-magi_20250502_v000.cdf", "rb"
        ) as output,
...
def test_calibration_creates_calibration_file():
    ...
    assert result.exit_code == 0
    assert Path("data/calibration.json").exists()
...
def test_application_creates_L2_file():
    ...
    print("\n" + str(result.stdout))
    assert result.exit_code == 0
    assert Path("data/L2.cdf").exists()
