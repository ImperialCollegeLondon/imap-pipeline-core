...
class TestSendMonthlyTestMessage:
    ...
    @pytest.mark.asyncio
    async def test_sends_notification_on_first_monday_of_month_with_no_previous_progress(self):
        ...
        with (
            ...
            patch("prefect_server.checkIALiRT.DatetimeProvider.now", return_value=first_monday),
            patch("prefect_server.checkIALiRT.MicrosoftTeamsWebhook.aload", return_value=mock_webhook),
        ):
    ...
    async def test_skips_notification_when_not_first_monday(self):
        ...
        with (
            patch("prefect_server.checkIALiRT.Database", return_value=mock_db),
            patch("prefect_server.checkIALiRT.DatetimeProvider.now", return_value=regular_tuesday),
        ):
            await send_monthly_test_message(logger, "test-webhook")
    ...
    async def test_skips_notification_when_already_sent_today(self):
        ...
        mock_progress = MagicMock()
        mock_progress.get_progress_timestamp.return_value = first_monday  # already sent today
    ...
    async def test_updates_last_checked_timestamp(self):
        ...
        mock_progress.update_last_checked_timestamp.assert_called_once_with(regular_tuesday)
...
class TestCheckIALiRTFlowUnit:
    ...
    async def test_flow_returns_failed_state_when_anomalies_found(self):
        ...
        with (
            patch("prefect_server.checkIALiRT.check_ialirt", return_value=[mock_anomaly]),
            patch("prefect_server.checkIALiRT.MicrosoftTeamsWebhook.aload", return_value=mock_webhook),
