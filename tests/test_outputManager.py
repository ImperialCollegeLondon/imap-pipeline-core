...
def test_copy_new_file(capture_cli_logs):
    ...
    # Verify.
    assert (
        f"Copied to {Path('output/hk/mag/l1/pwr/2025/05/imap_mag_l1_pwr_20250502_v000.txt')}."
        in capture_cli_logs.text
    )

    assert Path(
        "output/hk/mag/l1/pwr/2025/05/imap_mag_l1_pwr_20250502_v000.txt"
    ).exists()
...
def test_copy_file_same_content(capture_cli_logs):
    ...
    existing_file = create_test_file(
        Path("output/hk/mag/l1/pwr/2025/05/imap_mag_l1_pwr_20250502_v000.txt"),
        "some content",
    )
    ...
    # Verify.
    assert (
        f"File {Path('output/hk/mag/l1/pwr/2025/05/imap_mag_l1_pwr_20250502_v000.txt')} already exists and is the same. Skipping update."
        in capture_cli_logs.text
    )
...
def test_copy_file_second_existing_file_with_same_content(capture_cli_logs):
    ...
    # Verify.
    assert (
        f"File {Path('output/hk/mag/l1/pwr/2025/05/imap_mag_l1_pwr_20250502_v000.txt')} already exists and is different. Increasing version to 1."
        in capture_cli_logs.text
    )
    assert (
