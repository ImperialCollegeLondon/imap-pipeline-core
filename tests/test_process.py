...
def test_dispatch_unsupported_file(capture_logs):
    # Set up.
...
def test_decode_hk_packet_with_data_spanning_two_days(
    mock_met_to_j2000_conversion_for_hk_power_to_span_two_days, capture_logs
):
    ...
    assert "Generating file for 2025-05-02." in capture_logs.text
    assert "Generating file for 2025-05-03." in capture_logs.text


def test_decode_hk_packet_with_data_from_multiple_apids(capture_logs):
    """Test that HKProcessor splits data into separate files for each day, for each ApID."""
    ...
    assert f"Found 2 ApIDs (1063, 1064) in {packet_path}." in capture_logs.text
    assert (
        "Splitting data for ApID 1063 (MAG_HSK_PW) into separate files for each day:"
        in capture_logs.text
    )
