...
def test_upload_file_to_sdc(wiremock_manager, capture_logs):
    # Set up.
    ...
    # Verify.
    assert f"Uploading 1 files: {upload_file}" in capture_logs.text
    assert (
...
def test_failed_sdc_file_upload(wiremock_manager, capture_logs):
    # Set up.
...
@pytest.mark.asyncio
async def test_upload_flow_to_sdc(wiremock_manager, capture_logs):
    # Set up.
    upload_file = Path("imap_mag_l1c_norm-mago_20251017_v001.cdf")
    add_mapping_for_successful_sdc_upload(wiremock_manager, upload_file)
