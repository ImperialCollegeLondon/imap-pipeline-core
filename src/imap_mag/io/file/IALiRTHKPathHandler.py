...
@dataclass
class IALiRTHKPathHandler(IFilePathHandler):
    """
    Path handler for I-ALiRT MAG HK files.
    """

    root_folder: str = "ialirt_hk"
    ...
    def get_filename(self) -> str:
        ...
        return f"{self.mission}_ialirt_hk_{self.content_date.strftime('%Y%m%d')}.{self.extension}"
    ...
    @classmethod
    def from_filename(cls, filename: str | Path) -> "IALiRTHKPathHandler | None":
        match = re.match(
            r"imap_ialirt_hk_(?P<date>\d{8})\.(?P<ext>\w+)",
            Path(filename).name,
        )
        logger.debug(
            f"Filename {filename} matches {match.groupdict(0) if match else 'nothing'} with I-ALiRT HK regex."
        )
