...
T = typing.TypeVar("T", bound="SmallForcesPathHandler")

"""
Small forces files from the SDC API have paths like:
    imap/spice/activities/imap_2026_036_2026_037_hist_01.sff

They are saved under the spice folder in the datastore:
    [DATASTORE]/spice/activities/imap_2026_036_2026_037_hist_01.sff

The filename pattern is: imap_{start_year}_{start_doy}_{end_year}_{end_doy}_hist_{version}.sff
"""

SMALL_FORCES_FILENAME_PATTERN = re.compile(
    r"^imap_(\d{4})_(\d{3})_(\d{4})_(\d{3})_hist_(\d{2})\.sff$"
)


@dataclass
class SmallForcesPathHandler(VersionedPathHandler):
    """Path handler for small forces files."""
    ...
    def get_unsequenced_pattern(self) -> re.Pattern:
        raise ValueError(
            "Small forces files do not support sequencing. "
    ...
    def get_folder_structure(self) -> str:
        return (Path(self.get_root_folder()) / "activities").as_posix()
    ...
    def from_filename(cls: type[T], filename: str | Path) -> T | None:
        ...
        match = SMALL_FORCES_FILENAME_PATTERN.match(filename_only)
        if not match:
            return None
        ...
        logger.debug(
            f"Created SmallForcesPathHandler for file {filename} with version {version}."
        )
        return handler
