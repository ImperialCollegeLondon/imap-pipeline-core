...
class SPICEPathHandler(VersionedPathHandler):
    ...
    def parse_metakernel_kernels(metakernel_path: Path) -> list[str]:
        ...
        Each entry is the literal path from the metakernel, with any leading
        ``$SYMBOL/`` prefix (e.g. ``$KERNELS/``) stripped and any leading ``/``
        stripped.  For production metakernels generated by this codebase the
        entries are plain paths relative to the datastore root such as
        ``spice/ck/imap_dps_....bc``.
        """
