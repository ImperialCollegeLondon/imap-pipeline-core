...
class SPICEPathHandler(VersionedPathHandler):
    ...
    def parse_metakernel_kernels(metakernel_path: Path) -> list[str]:
        """Return the kernel paths referenced by a metakernel's ``KERNELS_TO_LOAD`` block.

        Each returned path is relative to the datastore's ``spice`` folder so
        callers can always prepend ``datastore / "spice"`` to locate the file.

        Two metakernel formats are normalised:

        - **Legacy / SDC format**: entries use a ``$SYMBOL/`` prefix (e.g.
          ``$KERNELS/lsk/naif0012.tls``). The ``$SYMBOL/`` segment is stripped,
          leaving ``lsk/naif0012.tls``.
        - **Production format** generated by ``generate_spice_metakernel``:
          entries are plain paths relative to the *datastore root* (e.g.
          ``spice/ck/imap_dps_....bc``). The leading ``spice/`` segment is
          stripped, leaving ``ck/imap_dps_....bc``.
        """
        ...
        for entry in entries:
            # Strip "$SYMBOL/" prefix (e.g. "$KERNELS/") used in legacy/SDC metakernels.
            path = re.sub(r"^\$\w+/", "", entry).lstrip("/")
            # Strip leading "spice/" from production metakernels (paths relative
            # to the datastore root) so the result is always relative to the
            # spice/ folder, consistent with the legacy format.
            path = re.sub(r"^spice/", "", path)
            result.append(path)
        return result
