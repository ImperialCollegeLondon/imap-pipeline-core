...
class PollSpinTableFlow:
    def __init__(self, datetime_provider: DatetimeProvider = DatetimeProvider()):
        self._datetime_provider = datetime_provider
        self.poll_spin_table_flow = flow(
            self._poll_spin_table_flow_impl,
    ...
    def _generate_flow_run_name(self) -> str:
        ...
        return (
            f"Download-SpinTable-from-{start_date}-to-{end_date.strftime('%d-%m-%Y')}"
        )

    async def _poll_spin_table_flow_impl(
        self,
...
_default_flow = PollSpinTableFlow()
poll_spin_table_flow = _default_flow.poll_spin_table_flow
