...
class PollSpiceFlow:
    def __init__(self, datetime_provider: DatetimeProvider = DatetimeProvider()):
        ...
        self.poll_spice_flow = flow(
            self._poll_spice_flow_impl,
    ...
    async def _poll_spice_flow_impl(
        self,
        ...
        check_timestamp = self._datetime_provider.now()
        ...
        automated_flow_run: bool = all(
            v is None for v in fields_all_none_means_automated
        )
        use_database: bool = force_database_update or automated_flow_run

        if ingest_start_day is not None and ingest_end_date is None:
            ingest_end_date = self._datetime_provider.tomorrow().date()
        ...
        if safe_download_dates[1] == self._datetime_provider.end_of_today():
            # SDC API is exclusive on end dates!
            safe_download_dates = (
                safe_download_dates[0],
                self._datetime_provider.tomorrow(),
            )
...
_default_flow = PollSpiceFlow()
poll_spice_flow = _default_flow.poll_spice_flow
