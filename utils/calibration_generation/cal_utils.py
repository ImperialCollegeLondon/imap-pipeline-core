...
variable_layers = [
    Path("utils/calibration_generation/cdf/imap_mag_ialirt-calibration_cdf_attrs.yaml"),
...
def generate_ialirt_file(
    version, valid_start_date, offsets, frame_transform_mago, frame_transform_magi
):
    cdf = pycdf.CDF(
        f"imap_mag_ialirt-calibration_{valid_start_date.strftime('%Y%m%d')}_v{version}.cdf",
        "",
    )
    ...
    setup_variable(
        ...
        "gradiometer_factor",
        0,
        var_type=pycdf.const.CDF_FLOAT,
        recVary=False,
    )
    setup_variable(
        cdf,
