"""
CREDIT TO https://github.com/IMAP-Science-Operations-Center/imap_processing/blob/61d71db2dfe32ca394673b3608014bf0a4824eb1/imap_processing/ccsds/excel_to_xtce.py
    ...
    def _create_container_sets(self) -> None:
        ...
        for packet_name, apid in self._packet_mapping.items():
            ...
            for i, row in packet_df.iterrows():
                ...
                original_name = f"{row['packetName']}.{row['mnemonic']}"
                if original_name in param_name_index_dict:
                    param_name_index_dict[original_name] += 1
                    name = f"{original_name}_{param_name_index_dict[original_name]}"
                else:
                    param_name_index_dict[original_name] = 1
                    name = original_name
                ...
                self._add_parameter(row, total_packet_bits, name)

    def _add_parameter(self, row: pd.Series, total_packet_bits: int, name: str) -> None:
        """
        ...
        if "UINT" in row["dataType"] or "FILL" in row["dataType"]:
            ...
            encoding.attrib["sizeInBits"] = str(length_in_bits)
            encoding.attrib["encoding"] = "IEEE754"
